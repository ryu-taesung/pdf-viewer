...
from PySide6.QtCore import (QObject, QRect, QRunnable, Qt, QThreadPool, Signal,
                            Slot)
from PySide6.QtGui import (QAction, QBrush, QColor, QIcon, QImage, QPalette,
                           QPixmap)
...
class CustomGraphicsView(QGraphicsView):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Left:
            ...
            self.viewer.center_action.setChecked(
                not self.viewer.center_action.isChecked())
            self.viewer.center_toggled()
        elif event.key() == Qt.Key_I:
            self.viewer.invert_action.setChecked(
                not self.viewer.invert_action.isChecked())
            self.viewer.show_page(self.viewer.current_page)
        elif event.key() == Qt.Key_R:
            if self.viewer.doc:
                self.viewer.show_page(
                    random.randint(
                        0, self.viewer.doc.page_count))
        elif event.key() == Qt.Key_T:
            if self.viewer.doc:
                self.viewer.two_pages_action.setChecked(
                    not self.viewer.two_pages_action.isChecked())
                self.viewer.show_page(self.viewer.current_page)
        elif event.key() == Qt.Key_Plus or event.key() == Qt.Key_Equal:
            if self.viewer.doc:
                self.viewer.zoom_edit.setText(
                    str(round(float(self.viewer.zoom_edit.text()) + self.zoom_step_size, 2)))
                self.viewer.show_page(self.viewer.current_page)
...
class Worker(QRunnable):
    ...
    def run(self):
        ...
        image = QImage.fromData(
            pdf_page.get_pixmap(
                matrix=mat).tobytes(), "PNG")
        image2 = None
        if pdf_page2:
            image2 = QImage.fromData(
                pdf_page2.get_pixmap(
                    matrix=mat).tobytes(), "PNG")
...
class PDFViewer(QMainWindow):
    def __init__(self):
        ...
        self.center_action = QAction(
            "Center Vertically", view_menu, checkable=True)
        self.invert_action = QAction(
            "Invert Colors", view_menu, checkable=True)
    ...
    def update_recent(self):
        recent_files = db_con.execute(
            "SELECT filename, page, last_accessed FROM memory ORDER BY last_accessed DESC LIMIT 36").fetchall()
        ...
        for file in recent_files:
            recent_action = self.recent_submenu.addAction(
                f'{file[0]} ({file[1]+1}); {str(datetime.fromtimestamp(file[2])).split(" ")[0]}')  # _path.basename(file[0]))
            # Connect the action to a function to handle opening recent PDFs
            recent_action.triggered.connect(create_recent_pdf_handler(file[0]))

    def open_pdf(self):
        options = QFileDialog.Option.ReadOnly
        file_name, _ = QFileDialog.getOpenFileName(
            self, "Open PDF", "", "PDF & Epub Files (*.pdf *.epub);;PDF Files (*.pdf);;Epub Books (*.epub);;All files (*.*)", options=options)
    ...
    def load_pdf(self, file_name):
        ...
        cur = db_con.cursor()
        cur.execute(
            "SELECT zoom, page, invert FROM memory WHERE filename LIKE ?",
            (self.file_name,
             ))
        row = cur.fetchone()
        if not row:
    ...
    def show_page(self, page_number):
        ...
        worker = Worker(
            ...
            float(
                self.zoom_edit.text()),
            self.invert_action.isChecked(),
            self.two_pages_action.isChecked())
    ...
    def update_memory(self):
        ...
        cur = db_con.cursor()
        cur.execute(
            "SELECT rowid FROM memory WHERE filename LIKE ?", (self.file_name,))
        row = cur.fetchone()
        if not row:
    ...
    def page_loaded(self, image1, image2=None):
        ...
        # If the second image is provided, create its QPixmap and add it to the
        # scene
        if image2:
            ...
            self.graphics_view.setSceneRect(
                0, 0, combined_width, combined_height)
        else:
        ...
        if self.center_action.isChecked():
            self.graphics_view.centerOn(
                self.graphics_scene.itemsBoundingRect().center())
        else:
    ...
    def next_page(self):
        if self.two_pages_action.isChecked() and self.current_page + \
                2 < self.doc.page_count:
            self.show_page(self.current_page + 2)
    ...
    def page_edit_changed(self):
        ...
        try:
            ...
            # Reset the page_edit to the current page number in case of invalid
            # input
            self.page_edit.setText(str(self.current_page + 1))
